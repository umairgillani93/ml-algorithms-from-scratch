...
struct Matrix *matmul(struct Matrix *matrix1, struct Matrix *matrix2) {
	...
	for (int i = 0; i < matrix1->rows; i++) {
		for (int j = 0; j < matrix1->cols; j++) {
			matrix1->data[i * matrix1->rows + j] = RAND_FLOAT;
		}
	}

	for (int i = 0; i < matrix2->rows; i++) {
		for (int j = 0; j < matrix2->cols; j++) {
			matrix2->data[i * matrix2->rows + j] = RAND_FLOAT;
		}
	}
	...
	for (int i = 0; i < matrix1->rows; i++) {
		for (int j = 0; j < matrix2->cols; j++) {
			...
			for (int k = 0; k < matrix1->cols; k++) {
				...
				sum += matrix1->data[i * matrix1->rows + k] * matrix2->data[k * matrix2->cols + j];
			}
			result -> data[i * matrix1 -> rows + j] = (float) sum;
		}
	}
	return result;
}
